...
def run_python_exec(code: str) -> dict:
    ...
    for token in forbidden:
        if token in code:
            return {"tool": "python_exec", "input": code,
                    "output": f"Forbidden: '{token}'", "success": False}
    ...
    try:
        ...
        output = buffer.getvalue().strip() or str(local_ns.get("result", "(no output)"))
        return {"tool": "python_exec", "input": code, "output": output, "success": True}
    except Exception as exc:
        return {"tool": "python_exec", "input": code,
                "output": f"{type(exc).__name__}: {exc}", "success": False}
    finally:
